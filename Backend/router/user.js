...
const { generateOTP } = require("./Extra/mail.js");
const VerificationToken = require("../Modals/VerificationToken.js");
const JWTSEC = "653eggs26537dhb";
const nodemailer = require('nodemailer');
const ResetToken = require("../Modals/ResetToken.js");
...
body('phonenumber').isLength({ min: 10 }),
 ...
 async(req,res)=>{
    ...
    const transport = nodemailer.createTransport({
      ...
      transport.sendMail({
        from:"Bhavikstackoverflow@gmail.com",
        to:user.email,
    ...
    res.status(200).json({Status:"Pending" ,msg:"please Check your mail",user:user._id})
// } catch (error) {
...
router.post("/forgot/password" , async(req,res)=>{
    ...
    if(token){
        return res.status(400).json("After one hour you can req another")
    }
    ...
    const transport = nodemailer.createTransport({
      ...
      transport.sendMail({
        ...
        to:user.email,
        subject:"Reset Token",
        html:`http://localhost:3000/website/reset/password?token=${RandomTxt}&_id=${user._id}`
...
router.put("/update/:id",verifyToken,async(req,res)=>{
    try {
        ...
        if(req.body.password){
            ...
            return res.status(400).json("You are not allowed to update the user details")
